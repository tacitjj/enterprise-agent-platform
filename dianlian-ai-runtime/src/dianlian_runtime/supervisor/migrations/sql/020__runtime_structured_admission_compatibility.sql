-- Purpose: admit and fence JAVA_CAPABILITY_STRUCTURED / 3.0 in the shared Run ledger.
-- Scope: exact Admission compatibility only; governed H12 / 2.2 remains unchanged.
-- Preconditions: migrations 000-019 are current and the routine owner remains sealed.
-- Activation: no Driver, route, Provider, UI, or role is enabled here.
...
ALTER TABLE deer_runtime.runtime_execution_admission_ref
    DROP CONSTRAINT ck_runtime_execution_admission_ref_contract_v22;
ALTER TABLE deer_runtime.runtime_execution_admission_ref
    ADD CONSTRAINT ck_runtime_execution_admission_ref_contract_supported CHECK (
...
CREATE OR REPLACE FUNCTION deer_runtime.admit_runtime_run(
    p_tenant_id UUID,
...
BEGIN
    ...
    IF p_admission_contract_version NOT IN ('2.2', '3.0') THEN
...
BEGIN
    IF p_runtime_version IS NULL
       ...
       OR p_admission_contract_version NOT IN ('2.2', '3.0') THEN
        RAISE EXCEPTION 'runtime_version, agent_name and a supported admission contract are required'
            USING ERRCODE = '22023';
    END IF;
...
CREATE OR REPLACE FUNCTION deer_runtime.load_runtime_execution_authority(
    p_tenant_id UUID,
    p_runtime_run_id UUID,
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1
         ...
         WHERE tenant_id = p_tenant_id
           AND runtime_run_id = p_runtime_run_id
           AND admission_contract_version IN ('2.2', '3.0')
    ) THEN
        RETURN;
    END IF;
